...
#if DEBUG_KCAL
	pr_info("KCALDebug - entering mdss_mdp_kcal_store_fb0_ctl\n");
#endif
...
#if DEBUG_KCAL
	pr_info("KCALDebug - entering mdss_mdp_kcal_display_commit\n");
#endif
...
static void mdss_mdp_kcal_update_pcc(struct kcal_lut_data *lut_data, bool from_external)
{
	u32 copyback = 0;
...
#if DEBUG_KCAL
	pr_info("KCALDebug - entering mdss_mdp_kcal_update_pcc\n");
#endif
	...
	if( from_external ) {
		...
		if ((pa_cfg_data->red == 0) || (pa_cfg_data->green == 0) || (pa_cfg_data->blue == 0)) {
			...
			if ((PCC_ADJ*(DEF_PCC - lut_data->red)) < pa_cfg_data->red) {
				pcc_config.r.r = pa_cfg_data->red - PCC_ADJ*(DEF_PCC - lut_data->red);
				pcc_config.r.r = (pcc_config.r.r < PCC_ADJ*lut_data->minimum) ? PCC_ADJ*lut_data->minimum : pcc_config.r.r;
			}
			else {
				pcc_config.r.r = PCC_ADJ*lut_data->minimum;
			}
			if ((PCC_ADJ*(DEF_PCC - lut_data->green)) < pa_cfg_data->green) {
				pcc_config.g.g = pa_cfg_data->green - PCC_ADJ*(DEF_PCC - lut_data->green);
				pcc_config.g.g = (pcc_config.g.g < PCC_ADJ*lut_data->minimum) ? PCC_ADJ*lut_data->minimum : pcc_config.g.g;
			}
			else {
				pcc_config.g.g = PCC_ADJ*lut_data->minimum;
			}
			if ((PCC_ADJ*(DEF_PCC - lut_data->blue)) < pa_cfg_data->blue) {
				pcc_config.b.b = pa_cfg_data->blue - PCC_ADJ*(DEF_PCC - lut_data->blue);
				pcc_config.b.b = (pcc_config.b.b < PCC_ADJ*lut_data->minimum) ? PCC_ADJ*lut_data->minimum : pcc_config.b.b;
			}
			else {
				pcc_config.b.b = PCC_ADJ*lut_data->minimum;
			}
		}
	}
	mutex_unlock(&pa_cfg_data->lock);
...
static void mdss_mdp_kcal_update_pa(struct kcal_lut_data *lut_data, bool default_val)
{
	u32 copyback = 0;
...
#if DEBUG_KCAL
	pr_info("KCALDebug - entering mdss_mdp_kcal_update_pa\n");
#endif
	...
	if (mdata->mdp_rev < MDSS_MDP_HW_REV_103) {
...
#endif
			pa_config.pa_data.hue_adj = 0x0;
			pa_config.pa_data.sat_adj = DEF_PA;
			pa_config.pa_data.val_adj = DEF_PA;
			pa_config.pa_data.cont_adj = DEF_PA;
		}
		else {
		...
		if( default_val ) {
			...
			pr_info("KCALDebug - mdss_mdp_kcal_update_pa, external\n");
#endif
			pa_v2_config.pa_v2_data.global_hue_adj = 0x0;
			pa_v2_config.pa_v2_data.global_sat_adj = DEF_PA;
			pa_v2_config.pa_v2_data.global_val_adj = DEF_PA;
			pa_v2_config.pa_v2_data.global_cont_adj = DEF_PA;
		}
		else {
		...
		if( default_val ) {
			...
			pr_info("KCALDebug - mdss_mdp_kcal_update_pa, external\n");
#endif
			payload->global_hue_adj = 0x0;
			payload->global_sat_adj = DEF_PA;
			payload->global_val_adj = DEF_PA;
			payload->global_cont_adj = DEF_PA;
		}
		else {
...
#if DEBUG_KCAL
	pr_info("KCALDebug - entering mdss_mdp_kcal_update_igc\n");
#endif
...
int mdp_pp_lut_set_external(struct mdp_pcc_cfg_data *config)
{
	...
	mutex_lock(&pa_cfg_data->lock);
#if DEBUG_KCAL
	pr_info("KCALDebug - entering mdp_pp_ioctl_external, r: %ul, g: %ul, b: %ul\n", config->r.r, config->g.g, config->b.b);
#endif
	pa_cfg_data->red = config->r.r;
	pa_cfg_data->green = config->g.g;
	pa_cfg_data->blue = config->b.b;
	mutex_unlock(&pa_cfg_data->lock);
	mutex_lock(&kcalld->lock);
	if (kcalld->lut_data->enable) {
		if ((PCC_ADJ*(DEF_PCC - kcalld->lut_data->red)) < config->r.r) {
			config->r.r -= PCC_ADJ*(DEF_PCC - kcalld->lut_data->red);
			config->r.r = (config->r.r < PCC_ADJ*kcalld->lut_data->minimum) ? PCC_ADJ*kcalld->lut_data->minimum:config->r.r;
		}
		else {
			config->r.r = PCC_ADJ*kcalld->lut_data->minimum;
		}
		if ((PCC_ADJ*(DEF_PCC - kcalld->lut_data->green)) < config->g.g) {
			config->g.g -= PCC_ADJ*(DEF_PCC - kcalld->lut_data->green);
			config->g.g = (config->g.g < PCC_ADJ*kcalld->lut_data->minimum) ? PCC_ADJ*kcalld->lut_data->minimum:config->g.g;
		}
		else {
			config->g.g = PCC_ADJ*kcalld->lut_data->minimum;
		}
		if ((PCC_ADJ*(DEF_PCC - kcalld->lut_data->blue)) < config->b.b) {
			config->b.b -= PCC_ADJ*(DEF_PCC - kcalld->lut_data->blue);
			config->b.b = (config->b.b < PCC_ADJ*kcalld->lut_data->minimum) ? PCC_ADJ*kcalld->lut_data->minimum:config->b.b;
		}
		else {
			config->b.b = PCC_ADJ*kcalld->lut_data->minimum;
		}
		mdss_mdp_kcal_update_pcc(kcalld->lut_data, false);
		mdss_mdp_kcal_display_commit();
		ret = 0;
	}
	mutex_unlock(&kcalld->lock);
#if DEBUG_KCAL
	pr_info("KCALDebug - exiting mdp_pp_ioctl_external, r: %ul, g: %ul, b: %ul\n", config->r.r, config->g.g, config->b.b);
#endif
	return ret;
}
						...
						const char *buf, size_t count)
{
	...
	struct lutd *kcald = dev_get_drvdata(dev);
	...
	kcald->lut_data->red = kcal_r;
	kcald->lut_data->green = kcal_g;
	kcald->lut_data->blue = kcal_b;
	mdss_mdp_kcal_update_pcc(kcald->lut_data, false);
	mdss_mdp_kcal_display_commit();
	mutex_unlock(&kcald->lock);
								...
								char *buf)
{
	...
	return scnprintf(buf, PAGE_SIZE, "%d %d %d\n",
		kcald->lut_data->red, kcald->lut_data->green, kcald->lut_data->blue);
}
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if ((r) || (kcal_enable != 0 && kcal_enable != 1) ||
		(kcald->lut_data->enable == kcal_enable)) {
		mutex_unlock(&kcald->lock);
		return -EINVAL;
	}

	kcald->lut_data->enable = kcal_enable;
	mdss_mdp_kcal_update_pcc(kcald->lut_data, !kcal_enable);
	mdss_mdp_kcal_update_pa(kcald->lut_data, !kcal_enable);
	mdss_mdp_kcal_display_commit();
	mutex_unlock(&kcald->lock);
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if ((r) || (kcal_invert != 0 && kcal_invert != 1) ||
		(kcald->lut_data->invert == kcal_invert)) {
		mutex_unlock(&kcald->lock);
		return -EINVAL;
	}
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	mutex_lock(&kcald->lock);
	kcald->lut_data->sat = kcal_sat;

	mdss_mdp_kcal_update_pa(kcald->lut_data, false);
	mdss_mdp_kcal_display_commit();
	mutex_unlock(&kcald->lock);
...
static int kcal_ctrl_probe(struct platform_device *pdev)
{
	...
	kcalld = devm_kzalloc(&pdev->dev, sizeof(*kcalld), GFP_KERNEL);
	if (!kcalld) {
		pr_err("%s: failed to allocate memory for kcalld\n",
			__func__);
		return -ENOMEM;
	}
	kcalld->lut_data = lut_data;

	platform_set_drvdata(pdev, kcalld);

	kcalld->lut_data->enable = 0x0;
	...
	pa_cfg_data = devm_kzalloc(&pdev->dev, sizeof(*pa_cfg_data), GFP_KERNEL);
	if( !pa_cfg_data) {
		pr_err("%s: failed to allocate memory for pa_cfg_data\n",
			__func__);
		return -ENOMEM;
	}
	pa_cfg_data->red = 0;
	...
	mutex_init(&pa_cfg_data->lock);

	mdss_mdp_kcal_update_pcc(kcalld->lut_data, false);
	mdss_mdp_kcal_update_pa(kcalld->lut_data, false);
	mdss_mdp_kcal_update_igc(kcalld->lut_data);
	mdss_mdp_kcal_display_commit();
...
{
	struct lutd *kcald = dev_get_drvdata(&pdev->dev);

	mutex_destroy(&kcald->lock);
	mutex_destroy(&pa_cfg_data->lock);
